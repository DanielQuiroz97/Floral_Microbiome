...
levels(micro_ps$sample_table$Treatment) <- list("Budding\nOptimum (100 mg/L N)" = "1_100", 
                                                "Budding\nHigh (200 mg/L N)" = "1_200", 
                                                "Budding\nLow (25 mg/L N)" = "1_25", 
                                                "Flowering\nOptimum (100 mg/L N)" = "2_100", 
                                                "Flowering\nHigh (200 mg/L N)" =  "2_200",
                                                "Flowering\nLow (25 mg/L N)" = "2_25")
...
ggsave(filename = "Figures/1_Petal_plot.jpg",  plot = plot_ps_venn, 
       width = 14, height = 12, units = "cm", dpi = 2000)
plot_ps_venn
```
...
fractionReadsAssigned.L<-filter(fractionReadsAssigned.L,taxlevel!="Kingdom")    
...
fraction <- ggplot(fractionReadsAssigned.L, aes(y = fractionReadsAssigned, x = taxlevel, fill=Treatment)) +
      theme_bw() +
      geom_boxplot(aes(), outlier.shape=NA, alpha=1) +  
      ylab("Fraction of Reads Classified") +
      xlab("Taxonomic level") + ylim(0,1) +
      ...
      scale_y_break(c(0.1,0.6)) + scale_y_break(c(0.75,0.85))

    ggsave(plot = fraction, filename = "Figures/1_Fraction of reads identified by taxonomic level.jpg", width = 170,
           height = 200,
           units = "mm",
           dpi = 2100)
fraction
