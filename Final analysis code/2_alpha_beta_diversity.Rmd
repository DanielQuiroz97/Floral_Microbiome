...
kruskal.test(Observed ~ Treatment, data = alpha_diversity)
kruskal.test(InvSimpson ~ Treatment, data = alpha_diversity)
kruskal.test(Shannon ~ Treatment, data = alpha_diversity)
pairwise.wilcox.test(alpha_diversity$Shannon, alpha_diversity$Treatment,
                 p.adjust.method = "none")
kruskal.test(Evenness ~ Treatment, data = alpha_diversity) 
pairwise.wilcox.test(alpha_diversity$Evenness, alpha_diversity$Treatment,
                 p.adjust.method = "none")
...
alpha_div_stats <- list("Observed\nKruskal-Wallis, p = 0.46", "Shannon\nKruskal-Wallis, p = 0.04", "InvSimpson\nKruskal-Wallis p = 0.06", "Evenness\nKruskal-Wallis, p = 0.01")
names(alpha_div_stats)<-c("Observed", "Shannon", "InvSimpson", "Evenness")
        
plot_alpha<-list()                
for(i in c("Observed", "Shannon", "InvSimpson", "Evenness")){

plot_alpha_diversity <- alpha_diversity_long %>% filter(.,AlphaDiversity == i)

plot_alpha[[i]] <- plot_alpha_diversity %>% 
          ggplot(aes(x=Treatment, y=AlphaDiversityMeasure)) + 
          geom_boxplot(aes(color=Time),size = 0.5)  + 
          #facet_wrap(~AlphaDiversity, ncol = 2, scales= "free",, labeller = labeller(AlphaDiversity=alpha_div_stats)) + 
          ylab("") + ylim(min(plot_alpha_diversity$AlphaDiversityMeasure)*0.95,max(plot_alpha_diversity$AlphaDiversityMeasure)*1.1) +
          ggtitle(alpha_div_stats[[i]])+
...
alpha_combine<-ggarrange(plot_alpha$Observed, plot_alpha$Shannon, plot_alpha$InvSimpson, plot_alpha$Evenness, ncol=2, nrow=2, common.legend = TRUE, legend="right")        
      
            ggsave(filename = "Figures/2c_fertilizer_alpha_time.jpg", plot = alpha_combine, 
               width = 170, height = 150, units = "mm", dpi = 2100)
plot_alpha
```
```{r}
